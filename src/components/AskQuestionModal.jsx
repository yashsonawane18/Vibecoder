...
export default function AskQuestionModal({ isOpen, onClose, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const createdQuestion = await onSubmit({
        title,
        body,
        subject,
        authorName,
      ...
      if (mode === 'ai' && createdQuestion?.aiAnswer) {
        setAiResult(createdQuestion);
      } else {
        handleClose();
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-bold text-gray-900">
            {aiResult ? '✨ Instant AI Answer' : 'Ask Your Academic Doubt'}
          </h2>
        ...
        {aiResult ? (
          <div className="p-6 space-y-5">
            <div className="p-4 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 border border-indigo-200 rounded-xl">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-2xl">🤖</span>
                  <div>
                    <h3 className="font-bold text-indigo-950 text-base">Gemini AI Solution</h3>
                    <p className="text-xs text-indigo-600">Generated for: &quot;{aiResult.title}&quot;</p>
                  </div>
                </div>
                <VoiceButton mode="speak" text={aiResult.aiAnswer} />
              </div>

              <div className="bg-white/80 p-4 rounded-lg text-sm text-gray-800 leading-relaxed max-h-96 overflow-y-auto whitespace-pre-wrap border border-indigo-100">
                {aiResult.aiAnswer}
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
              <a
                href={`/question/${aiResult.id}`}
                className="text-sm font-semibold text-indigo-600 hover:text-indigo-800 flex items-center gap-1"
              >
                Open Full Question Thread & Answers →
              </a>
              <button
                type="button"
                onClick={handleClose}
                className="w-full sm:w-auto bg-indigo-600 text-white px-6 py-2.5 rounded-lg font-medium shadow hover:bg-indigo-700 transition-colors"
              >
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="flex justify-end pt-4 border-t gap-3 items-center">
              <button
                type="button"
                onClick={handleClose}
                disabled={isSubmitting}
                className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 border border-gray-300 rounded-md shadow-sm disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={isSubmitting}
                className="btn-primary bg-indigo-600 text-white px-5 py-2 rounded-md shadow-sm hover:bg-indigo-700 font-medium flex items-center gap-2 disabled:opacity-75"
              >
                {isSubmitting ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    <span>{mode === 'ai' ? '🤖 Generating AI Answer...' : 'Posting...'}</span>
                  </>
                ) : (
                  <span>{mode === 'ai' ? '🤖 Get Instant AI Answer' : '👨‍🏫 Post to Mentors Forum'}</span>
                )}
              </button>
            </div>
          </form>
